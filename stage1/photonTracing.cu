...
__global__ void getPhotonsKernel(SceneInfo scene, Photon* photonList, uint numPhotons) {
	...
	float3 color = scene.lights[0].intensity * 100000;
	...
	for (uint i = 0; i < count; ++i) {
		auto p = photonList[MAX_DEPTH*idx + i].power;
		// printf("%f,%f,%f\n", p.x, p.y, p.z);
		photonList[MAX_DEPTH*idx + i].power /= count;
	}
}

void writeTestToFile(const std::vector<Photon>& photons, const std::string& filename) {
...
std::vector<Photon> tracePhotons(const SceneInfo& scene, uint numPhotons) {
